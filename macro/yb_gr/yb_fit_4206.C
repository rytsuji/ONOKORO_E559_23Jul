{
  ...
  TString fname= "macro/yb_gr/yb_4206.dat";
  ofstream ofile((std::string) fname);
  
  TString filename="macro/yb_gr/x_4206.dat";
  ...
  while(getline(ifile,iline)){
    ...
    TCut acut="abs(vdc_gr_m.fA*1000)<2.5";
    TCut xcut=Form("abs(vdc_gr_m.fX-%e)<%e",mean,1.0*sigma);

    c1->cd();
    c1->cd(i+1);
    gPad->SetLeftMargin(0.15);
    h[i]=new TH2F(Form("h[%d]",i),Form("Maylar(p,p) GR Y vs B @X=%e (mm), |fA|< 2.5 (mrad)",mean),200,-50,50,200,-50,50);
  ...
  c3->SaveAs("figs/output_macro/yb_gr/yb_fit_4206.pdf");
  c3->SaveAs("figs/output_macro/yb_gr/yb_fit_4206.png");
  ofile.close();
}
