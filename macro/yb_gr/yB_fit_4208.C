{
  ...
  TString fname= "macro/y_correction_gr/xB_4208.dat";
  ofstream ofile((std::string) fname);
  
  TString filename="macro/y_correction_gr/x_4208.dat";
  ...
  while(getline(ifile,iline)){
    ...
    ofile << mean << " " << -1.0/p1 << " " << -p0/p1 << std::endl;
