{
  ...
  double BC_mylar=48528.6+80088.6;
  double BC_carbon=106791.2; 
  double BC_ca=0.2*(1258513.+1230251.+1240575.);
  double scale=-1.0/11.0;
  
  TFile *file_mylar = TFile::Open("output/phys/run4136.hist.root");
  ...
  TFile *file_mylar1 = TFile::Open("output/phys/run4137.hist.root");
  TDirectoryFile* dir_mylar1 = (TDirectoryFile*)file_mylar1->Get("phys");
  TH1F* ht_mylar1=(TH1F*)dir_mylar1->Get("sx_tc");
  TH1F* ha_mylar1=(TH1F*)dir_mylar1->Get("sx_ac");

  
  TFile *file_carbon = TFile::Open("output/phys/run4135.hist.root");
  ...
  sx_mylar->Add(ht_mylar1,1.0);
  sx_mylar->Add(ha_mylar1,scale);
  ...
  TString file_root[n_file]={"run4139.hist.root",
			     "run4140.hist.root",
			     "run4141.hist.root"};
