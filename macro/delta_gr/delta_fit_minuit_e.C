...
void chi2(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag){
  ...
  for(int i=0; i<loop; i++){
    ...
    */
    ...
    double Delta=elastic(par[15],33.3+data[3][i],(int) data[4][i],data[5][i]);
    //double Delta=data[2][i];
    chisq += pow(Delta-func,2.0);
    // }
...
int delta_fit_minuit_e(){
  ...
  while(getline(infile,iline)){
    ...
    std::string str[6];
    line >> str[0] >> str[1] >> str[2] >> str[3] >> str[4] >> str[5];
    ...
    data[4].push_back(std::stod(str[4])); //target
    data[5].push_back(std::stod(str[5])); //Q
  }
  
  
  //TMinuitクラスをインスタンス化
  TMinuit *min = new TMinuit(16);//TMinuit(n) n:パラメータ数
  min->SetPrintLevel(1);
  ...
  TH2F *hd = new TH2F("hd","hd",100,-0.1,0.1,500,-0.01,0.01);
  ...
  for(int i=0;i<loop;i++){
    ...
    Double_t delta=elastic(par[15],33.3+data[3][i],(int) data[4][i],data[5][i]);
    //Double_t delta=data[2][i];
      
    hist->Fill(delta-val);
    ht->Fill(data[3][i],delta-val);
    hx->Fill(data[0][i],delta-val);
    ha->Fill(data[1][i],delta-val);
    hd->Fill(delta,delta-val); 
  }
