...
void delta_calib_4208(){  

  TFile *_file0 = TFile::Open("output/run4208.root");
  
  //file io
  TString dir = "/home/tsuji/work/art_analysis/e559_23jul/macro/delta_gr/dat/";
  TString iname = "/home/tsuji/work/art_analysis/e559_23jul/macro/delta_gr/fig/4208_Ca_gs";
  TString oname = dir + "4208_Ca_gs";
  ...
  for(int i=0;i<9;i++){
    ...
    x[i]=xc;
    ac[i]=a+b*theta;
    delta[i]=(elastic(33.3+theta,3,0)-p0)/p0;
    th[i]=theta;

  }

  for(int i=0;i<9;i++){
    ofile << x[i] <<" "<< ac[i] <<" "<< delta[i] <<" "<< th[i] << " 3 0"<<std::endl;
  }
