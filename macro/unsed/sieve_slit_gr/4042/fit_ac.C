void fit_ac(int i_a){
  ...
  TF1 *g5 = new TF1("g5","f0+f1+f2+[9]+[10]*x+[11]*x*x",-50,50);


  Double_t ysigma=2.0;
  //Double_t yc[5]={-10, -2.5, 5.0, 10, 15}; //for 4040 a5
  Double_t yc[3]={ -2.5, 7.5, 15.}; //for 4040 a1-a4
  Double_t yh[3]={1000.,2500., 1000.};
  ...
  g5->SetParameter(9,0);
  g5->SetParameter(10,0);
  g5->SetParameter(11,0);
  ...
  for(int i=0;i<3;i++){
    ofile << dtheta*((double) -i_a+3) << " " << dphi*((double)i-1)<< " " << x_fp[i] << " " << x_fp_e[i] << " " << a_fp[i] << " " << a_fp_e[i] << " " << y_fp[i] << " " << y_fp_e[i] << " " << b_fp[i] << " " << b_fp_e[i] << std::endl;
  }
