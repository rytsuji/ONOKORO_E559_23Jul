{
  ...
  TCut cut="abs(vdc_gr_m.fX-200)<150 && abs(vdc_gr_m.fY-5)<25 ";
  TCut phicut="abs(phi_gr/1000*TMath::RadToDeg())<0.5";  
  TH1F *h_theta=new TH1F("h_theta","theta GR (deg)",800,-4,4);
  tree->Draw("theta_gr/1000.*TMath::RadToDeg()>>h_theta",cut && phicut,"");
  ...
  for(int i=0;i<5;i++){
    std::cout << "peak: " << g5->GetParameter(3*i+1)/(7.55/1000*TMath::RadToDeg()) << std::endl;;
  }
