...
void chi2(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag){
  ...
  for(int i=0; i<loop; i++){
    ...
    func2 = par[4]*data[0][i]*data[1][i]+par[5]*data[1][i]*data[2][i]+par[6]*data[2][i]*data[0][i];
    func3 = par[7]*data[0][i]*data[1][i]*data[2][i];
...
int phi_fit_minuit(){
  ...
  //fit結果を入れておくための変数
  TString oname="/home/tsuji/work/art_analysis/e559_23jul/macro/sieve_slit_gr_bac/gr_phi_calib.dat";
  ...
  TH1F *h1 = new TH1F("h1","h1",100,-50,50);
  int loop =  data[0].size();
  
  
  for(int i=0;i<loop;i++){
    double val1 = par[0]+par[1]*data[0][i]+par[2]*data[1][i]+par[3]*data[2][i];
    double val2 = par[4]*data[0][i]*data[1][i]+par[5]*data[1][i]*data[2][i]+par[6]*data[2][i]*data[0][i];
    double val3 = par[7]*data[0][i]*data[1][i]*data[2][i];
