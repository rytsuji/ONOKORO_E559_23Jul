...
#include "TVDCFPTrackingProcessor.h"
...
ClassImp(art::TVDCFPTrackingProcessor)

art::TVDCFPTrackingProcessor::TVDCFPTrackingProcessor()
{
  ...
  Register(fFPTiltedAngleY("FPTiltedAngleY","tilted angle of fp  along y axis in deg",45));
  //Register(fTiltedVDCAngleX("VDCTiltedAngleX","tilted angle of whole system along x axis in deg",0));                                                 
  Register(fVDCTiltedAngleY("VDCTiltedAngleY","tilted angle of whole system along y axis in deg",0));
  //Register(fTiltedVDCAngleZ("VDCTiltedAngleZ","tilted angle of whole system along z axis in deg",0));  
}
...
void art::TVDCFPTrackingProcessor::Process()
{
...
void art::TVDCFPTrackingProcessor::ProcessOne(const std::vector<TVDCCluster*>& planes)
{
...
void art::TVDCFPTrackingProcessor::FindTrack(const std::vector<TVDCCluster*>& planes, TMWDCTrackingResult *tr, bool doFillStat)
{
  ...
  // Info("FindTrack","enter");
   ...
   if (fNParameter == 2) {
     ...
     double PosZfp=(tan(-fVDCTiltedAngleY*TMath::DegToRad())*(bestTrackPrm[0]+fOffsetX)-fOffsetZ)/(1-tan(-fVDCTiltedAngleY*TMath::DegToRad())*vec.X());
     //TVector3 pos(bestTrackPrm[0]+vec.X()*PosZfp+fOffsetX,
     //bestTrackPrm[1]+vec.Y()*PosZfp+fOffsetY,
     //PosZfp+fOffsetZ);
     double Xfp=(bestTrackPrm[0]+vec.X()*PosZfp+fOffsetX)*cos(fVDCTiltedAngleY*TMath::DegToRad())-(PosZfp+fOffsetZ)*sin(fVDCTiltedAngleY*TMath::DegToRad());
     double Yfp=bestTrackPrm[1]+vec.Y()*PosZfp+fOffsetY;
     double Zfp=(bestTrackPrm[0]+vec.X()*PosZfp+fOffsetX)*sin(fVDCTiltedAngleY*TMath::DegToRad())+(PosZfp+fOffsetZ)*cos(fVDCTiltedAngleY*TMath::DegToRad());

     //pos.RotateY(fVDCTiltedAngleY*TMath::DegToRad());
     vec.RotateY((fFPTiltedAngleY-fVDCTiltedAngleY)*TMath::DegToRad());
     
     
     //tr->SetTrack(bestTrackPrm[0],bestTrackPrm[1],0,
     //tr->SetTrack(pos.X(),pos.Y(),pos.Z(),
     tr->SetTrack(Xfp,Yfp,Zfp,
		  vec.X()/vec.Z(),
		  vec.Y()/vec.Z());
   }
   tr->SetSSR(SSR);
