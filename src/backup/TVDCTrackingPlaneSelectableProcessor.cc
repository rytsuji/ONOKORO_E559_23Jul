/**
 * @File   TVDCTrackingClustSizeSelectableProcessor.h
...
#include "TVDCTrackingPlaneSelectableProcessor.h"
...
ClassImp(art::TVDCTrackingPlaneSelectableProcessor)

art::TVDCTrackingPlaneSelectableProcessor::TVDCTrackingPlaneSelectableProcessor()
...
void art::TVDCTrackingPlaneSelectableProcessor::Process()
{
...
//   Info("Process","========== New Event ==========");
   ...
   for (int iPlane = 0; iPlane < fNPlane; ++iPlane) {
	...
	printf("test : plane=%d, iHit=%d ,clusternum=%d\n",iPlane,iHit,planeCluster->GetClustnum());
	//	 printf("iPlane = %d, iHit = %d, %p\n",iPlane,iHit,planeCluster);
	 ...
	 if (fVerboseLevel > 2)  printf("Hit Pos = %10f,  Timestamp = %20f\n",planeCluster->GetHitPos(),planeCluster->GetTimestamp());
         //if (iPlane == 0 ) {
	 if (firsthit == 0 && planeCluster->GetClustnum() > 2) {
	   ...
	   if (abs(planeCluster->GetTimestamp() - aSet[0]->GetTimestamp()) < fSearchTimeWidth.Value() && planeCluster->GetClustnum() > 2) {
...
void art::TVDCTrackingPlaneSelectableProcessor::ProcessOne(const std::vector<TVDCCluster*>& planes)
{
...
void art::TVDCTrackingPlaneSelectableProcessor::FindTrack(const std::vector<TVDCCluster*>& planes, TMWDCTrackingResult *tr, bool doFillStat)
{
  ...
  std::vector<Double_t> pos(PlaneSize), trackPrm(fNParameter);
  std::vector<Double_t> bestTrackPrm(fNParameter,0);
  std::vector<Double_t> residual(PlaneSize,0);
  std::vector<Double_t> bestResidual(PlaneSize,0);
...
//      vec.Print();
      ...
      vec.RotateX(fTiltedAngleX*TMath::DegToRad());      
      vec.RotateY(fTiltedAngleY*TMath::DegToRad());
      vec.RotateZ(fTiltedAngleZ*TMath::DegToRad());//      vec.Print();
